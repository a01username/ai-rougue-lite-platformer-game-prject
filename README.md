...
River challenge and mirrored rooms: L rooms now generate with either a left or right shaft. Their bottom entrance aligns with the shaft and their upper exit with the far end of the horizontal arm, so the route crosses the bend. Water flows toward the shaft in either orientation; river force is 5200 world units/s² and waterfall force 4200 world units/s². Maps reflect the room orientation. Checked 206 generated L rooms for route clearance, entrance/exit placement, mirrored currents, and rendering.

Extended L rooms: generation now favors six L variants, including mirrored standard (2x2), long-arm (3x2), and tall-shaft (2x3) footprints. They replace eligible room choices rather than adding room-count steps. Full-footprint occupancy checks prevent overlaps; lower entrances remain in the shaft and upper exits over the far arm. Rock corners, river lengths, waterfall heights, platform routes, and map silhouettes scale with the chosen footprint. Verified 499 L rooms across 60 generated floors with all six variants present and no route/entrance/exit/current check failures.
